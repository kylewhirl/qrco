...
import { iconsData } from "../icons-data/icons-data";
...
const IconRenderer = React.memo(({ name, color }: { name: string; color?: string }) => {
  ...
  const iconsToUse = useMemo(() => {
    ...
    return source.filter(icon => {
      ...
      if (icon.name.startsWith('Si')) {
        return Boolean((SiIcons as Record<string, unknown>)[icon.name]);
      }
      return true;
    });
  }, [iconsList, icons]);
    ...
    <TooltipProvider key={icon.name}>
      <Tooltip>
        <TooltipTrigger
          ...
          <IconRenderer name={icon.name} color={(icon as IconData).hex} />
        </TooltipTrigger>
        <TooltipContent>
            <p>{icon.name.replace(/^Si/, "")}</p>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  ), [handleIconClick]);
            ...
            {(value || selectedIcon) ? (
              <>
                <Icon
                  ...
                  color={iconsData.find(i => i.name === (value || selectedIcon))?.hex}
                /> {value || (selectedIcon ?? "").replace(/^Si/, "")}
              </>
            ) : (
              triggerPlaceholder
            )}
...
const Icon = React.forwardRef<
  React.ComponentRef<LucideIcon | React.ComponentType>,
  IconProps
>(({ name, ...props }, ref) => {
  // Try React-Icons Si first
  const SiComponent = (SiIcons as Record<string, React.ComponentType<{ size?: number; color?: string; ref?: React.Ref<SVGSVGElement> }>>)[name];
  if (SiComponent) {
    // Only pass size and color to React-Icons
    const { size, color, ...rest } = props as { size?: number; color?: string };
    return <SiComponent size={size} color={color} ref={ref as React.Ref<SVGSVGElement>} {...rest} />;
  }
  return <DynamicIcon name={name as IconName} {...props} ref={ref as React.Ref<SVGSVGElement>} />;
});
